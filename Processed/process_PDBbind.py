"""
Extracts protein and ligand .mol2 files from PDBbind dataset folders.
"""
...
def extract_protein_and_ligand(protein_folder: str, output_base_path: str) -> None:
...
def process_protein(protein_directory: str, output_base_path: str) -> None:
    ...
    for protein_folder in protein_folders:
        extract_protein_and_ligand(protein_folder, output_base_path)


def main() -> None:
    """Parse command-line arguments and process protein directory."""
    if len(sys.argv) != 3:
        print("Usage: python process_pdbbind.py <protein_folder> <output_folder>")
        sys.exit(1)
